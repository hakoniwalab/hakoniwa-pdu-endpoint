...
def explain_field(key: str) -> str:
    explanations = {
        ...
        "protocol": "Protocol selects the delivery or persistence backend and must be explicit.",
        ...
        "storage": "Storage settings define how PDU packets are persisted and replayed.",
        "backend": "Storage backend must be explicit so persistence behavior is stable and inspectable.",
        "mode": "Mode selects snapshot semantics (`latest`) or append-only replay semantics (`queue`).",
        "path": "The output or input file path must be explicit so persisted state is auditable.",
        "comm_raw_version": "Raw packet version defines the on-disk framing used by storage transport.",
        "zenoh": "Zenoh settings split native transport/session config from Hakoniwa-specific callback semantics.",
        "key_prefix": "key_prefix defines the mapping from PDU keys to Zenoh key expressions.",
        "io": "io enumerates per-key callback semantics such as notify_on_recv.",
        "notify_on_recv": "notify_on_recv explicitly controls callback-driven delivery for a key.",
        "nodeId": "nodeId groups endpoints under a single container identity.",
        "endpoints": "Container endpoints must be enumerated explicitly to preserve orchestration semantics.",
        "id": "Container endpoint ids provide stable references inside the node.",
    }
