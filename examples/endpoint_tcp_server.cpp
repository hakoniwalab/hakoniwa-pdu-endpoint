...
int main() {
    ...
    if (server.open("config/sample/endpoint_tcp_server.json") != HAKO_PDU_ERR_OK) {
        std::cerr << "Failed to open TCP server endpoint" << std::endl;
        return 1;
    }
    if (server.start() != HAKO_PDU_ERR_OK) {
        std::cerr << "Failed to start TCP server endpoint" << std::endl;
        return 1;
    }
