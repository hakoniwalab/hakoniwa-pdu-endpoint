...
Python CDR examples are also available after running the `rmw_zenoh`
installer:
...
For a manual build, build the shared native library and the Python CFFI module:
...
Set the runtime paths, or source the `rmw-zenoh.env` generated by the
installer, then run the samples:
...
python3 python/examples/endpoint_rmw_zenoh_sub_cdr.py config/sample/endpoint_rmw_zenoh_sub.json
python3 python/examples/endpoint_rmw_zenoh_pub_cdr.py config/sample/endpoint_rmw_zenoh_pub.json
```

The Python examples use generated registry modules such as
`python/std_msgs/pdu_cdr_conv_UInt64.py` from `HAKO_PDU_REGISTRY_PDU_PATH`.
