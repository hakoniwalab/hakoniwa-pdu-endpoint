...
int main()
{
    hakoniwa::pdu::Endpoint endpoint("mqtt_pub_example", HAKO_PDU_ENDPOINT_DIRECTION_OUT);
    if (endpoint.open("config/sample/endpoint_mqtt_pub.json") != HAKO_PDU_ERR_OK) {
        std::cerr << "Failed to open MQTT publisher endpoint" << std::endl;
        return 1;
    }
    if (endpoint.start() != HAKO_PDU_ERR_OK) {
        std::cerr << "Failed to start MQTT publisher endpoint" << std::endl;
        return 1;
    }
    ...
    for (std::uint64_t i = 1; i <= 5; ++i) {
        ...
        if (endpoint.send(key, payload) != HAKO_PDU_ERR_OK) {
            std::cerr << "MQTT send failed" << std::endl;
            return 1;
        }
