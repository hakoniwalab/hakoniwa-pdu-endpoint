...
int main() {
    ...
    if (client.open("config/sample/endpoint_websocket_client.json") != HAKO_PDU_ERR_OK) {
        std::cerr << "Failed to open WebSocket client endpoint" << std::endl;
        return 1;
    }
    if (client.start() != HAKO_PDU_ERR_OK) {
        std::cerr << "Failed to start WebSocket client endpoint" << std::endl;
        return 1;
    }
